...
local backpackRunning = false
local buySweepActive = false
local function StartAutoBackpack()
	if backpackRunning then return end
	backpackRunning = true
	task.spawn(function()
		local failStreak = 0
		while Toggles["AutoBackpack"] and getgenv().__MS_Gen == myGen do
			if buySweepActive then
				task.wait(1)
			elseif Toggles["AutoSell"] and GetInventoryAmount() >= SellTreshold then
				lastBoughtPackText = "waiting for sell..."
				task.wait(1)
			elseif sellTrip then
				task.wait(0.5)
			else
				buySweepActive = true
				local coinsBefore = GetCoinsAmount()
				for i = 3, 50 do
					if not Toggles["AutoBackpack"] then break end
					if sellTrip then break end
					if not Remote then EnsureRemote() end
					if Remote then pcall(function() Remote:FireServer("BuyItem", {{"Backpack", i}}) end) end
					lastBoughtPackText = "Pack #" .. tostring(i)
					task.wait(0.3)
				end
				buySweepActive = false
				pcall(function() gearPackText = "max " .. tostring(select(2, GetInventoryAmount())) end)
				if GetCoinsAmount() < coinsBefore then
					failStreak = 0
					task.wait(3)
				else
					failStreak = failStreak + 1
					task.wait(math.min(5 + failStreak * 5, 30))
				end
			end
		end
		backpackRunning = false
	end)
end

local toolsRunning = false
local function StartAutoTools()
	if toolsRunning then return end
	toolsRunning = true
	task.spawn(function()
		local failStreak = 0
		while Toggles["AutoTools"] and getgenv().__MS_Gen == myGen do
			...
			elseif sellTrip then
				...
				for i = 1, 50 do
					...
					if sellTrip then break end
					if not Remote then EnsureRemote() end
					if Remote then pcall(function() Remote:FireServer("BuyItem", {{"Tools", i}}) end) end
					lastBoughtToolText = "Tool #" .. tostring(i)
					task.wait(0.3)
				end
				buySweepActive = false
				pcall(function() gearToolText = bestToolName() end)
				if GetCoinsAmount() < coinsBefore then
					failStreak = 0
					task.wait(3)
				else
...
MiscTab:Toggle({
	Title = "Auto Backpack",
	Desc = "Fires BuyItem for packs 3-50 on loop",
	Value = false,
	Callback = function(state)
...
MiscTab:Toggle({
	Title = "Auto Tools",
	Desc = "Fires BuyItem for tools 1-50 on loop",
	Value = false,
	Callback = function(state)
...
task.spawn(function()
	while Window and getgenv().__MS_Gen == myGen do
		pcall(function()
			...
			MineStatus:SetDesc(string.format("depth %s / target %s%s%s%s%s", tostring(curDepth), tostring(Depth), Toggles["AutoMine"] and " MINE" or "", Toggles["FastMine"] and " FAST" or "", Toggles["AutoRebirth"] and (" REBIRTH:" .. tostring(rebirthPhaseText)) or "", areaTransit and " AREA" or ""))
